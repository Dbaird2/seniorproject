...
try {
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (!empty($email) && !empty($pw)) {
            ...
            $info = $query_repo->fetchOne($stmt, $email);
            if (!$info) {
                 ...
                 */
                    ...
                    $audit_check = $query_repo->fetchOne($update_audit_id_q);
                    ...
                    if ($audit_check) {
                        if ((int)$audit_check['spa_check'] >= 0) {
                            ...
                            $params[] = $new_spa_id;
                            $update_c[] = 'curr_spa_id = ?';

                            $update_history_q = "UPDATE audit_history SET audit_id = 9 WHERE audit_id = ? AND audit_statis = 'In Progress'";
                            $query_repo->execute($update_history_q, $audit_check['curr_spa_id']);
                        }
                        if ((int)$audit_check['self_check'] >= 0) {
                            $new_self_id = (int)$audit_check['curr_self_id'] === 1 ? 2 : 1;
                            $params[] = $new_self_id;
                            $update_c[] = 'curr_self_id = ?';

                            $update_history_q = "UPDATE audit_history SET audit_id = 3 WHERE audit_id = ? AND audit_statis = 'In Progress'";
                            $query_repo->execute($update_history_q, $audit_check['curr_self_id']);
                        }
                        if ((int)$audit_check['mgmt_check'] >= 0) {
                            $new_mgmt_id = (int)$audit_check['curr_mgmt_id'] === 4 ? 5 : 4;
                            $params[] = $new_mgmt_id;
                            $update_c[] = 'curr_mgmt_id = ?';

                            $update_history_q = "UPDATE audit_history SET audit_id = 6 WHERE audit_id = ? AND audit_statis = 'In Progress'";
                            $query_repo->execute($update_history_q, $audit_check['curr_mgmt_id']);
                        }
                        if (!empty($update_c)) {
                            $ready_columns = implode(',', $update_c);
                            $update_id = "UPDATE audit_freq SET $ready_columns";
                            $query_repo->execute($update_id, ...$params);
                        }
                    }
                    $query = "UPDATE user_table SET last_login = CURRENT_TIMESTAMP WHERE email = ?";
                    if ($query_repo->execute($query, $user_check['email'])) {
