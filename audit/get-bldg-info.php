<?php
require_once "../config.php";
check_auth();
try {
    $decoded_data = file_get_contents('php://input');
    $data = json_decode($decoded_data, true);

    $select = "SELECT b.bldg_id, b.bldg_name, r.room_tag, r.room_loc from room_table r left join bldg_table b on r.bldg_id = b.bldg_id where room_tag = ?";
    try {

        $bldg_data = $query_repo->fetchOne($select, $data['room_tag']);
    } catch (PDOException $e) {
        echo json_encode(['bldg_name' => '', 'status' => 'failed' . $e->getMessage()]);
        exit;
    }
} catch (PDOException $e) {
    error_log($e->getMessage());
} catch (Exception $e) {
    error_log($e->getMessage());
}
echo json_encode(["bldg_name" => $bldg_data['bldg_name'] ?? null, "room_number" => $bldg_data['room_loc'] ?? null, 'bldg_id' => $bldg_data['bldg_id'] ?? null]);
exit;
